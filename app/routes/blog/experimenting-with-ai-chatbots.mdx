...
My neighbor [Padraig](https://www.instagram.com/retrored1) and I figured that a chatbot would be a good way to communicate with GPT-3, so we came up with a few concepts:
...
I used [Stable Diffusion](https://huggingface.co/spaces/stabilityai/stable-diffusion) to create the icon for Talk to Garth. The prompt was something like "A southern cowboy in the style of Disney".

<img src="/images/garth.png" className="rounded-xl" width="80" height="80" />
...
Here's the API request that gets sent to OpenAI. You'll see that I also pass in the most recent few chats so that the AI has some conversational context:
...
Here's the prompt in a more readable format. Essentially we're giving GPT-3 some background and telling it how we want it to respond:
...
It'll get you started with a GPT-3 powered chatbot that deploys to Cloudflare's edge network all over the world.
